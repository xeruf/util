...
fun Double.format(digits: Int, locale: Locale = Locale.ENGLISH) =
...
tailrec fun Int.factorial(total: Double = 1.0): Double =
	if(this <= 1) total else (this - 1).factorial(total * this)

/** @return The factorial of this Int down to [downTo].
 * For example `5.factorial(3)` would return 60. */
tailrec fun Int.factorial(downTo: Int, total: Double = 1.0): Double =
	if(this <= downTo) total else (this - 1).factorial(downTo, total * this)
